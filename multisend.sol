...
contract UpgradebleStormSender is OwnedUpgradeabilityStorage, Claimable {
    ...
    address public _contAddr;
    address public _senderAddr;
 //   address public _callerBa;
    uint256 _change;
    uint256 sentTokens=0;
    uint256 totalTokenTransferred=0;
    
    function ShowContAddr() external view returns(address) {
        return _contAddr;
    } 
    
    function ShowSenderAddr() external view returns(address) {
        return _senderAddr;
    } 
    
    constructor() public {
        assembly {
            sstore(_contAddr_slot, address)
        }
    }
